...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def MostRet(Twit1: Tweet, Twit2: Tweet): Tweet = {
      if (Twit1.retweets > Twit2.retweets) Twit1 else Twit2
    }
    if (left.isEmpty && right.isEmpty) elem
    else if (right.isEmpty) MostRet(left.mostRetweeted, elem)
    else if (left.isEmpty) MostRet(right.mostRetweeted, elem)
    else MostRet(left.mostRetweeted, MostRet(left.mostRetweeted, elem))
  }
